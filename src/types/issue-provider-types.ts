// Local shim for issue-provider plugin types.
//
// The Super Productivity monorepo's packages/plugin-api/src/index.ts exports
// these from `./issue-provider-types`, but the npm-published
// @super-productivity/plugin-api@1.0.1 predates that export (only
// johannesjo, the upstream package owner, can publish a fix). Since these are
// compile-time-only types with no runtime code, vendoring them here carries
// no behavioral risk. Copied verbatim from the monorepo's
// issue-provider-types.ts — drop this file and import from
// '@super-productivity/plugin-api' again once a release includes it.

// OAuthFlowConfig isn't published in @super-productivity/plugin-api@1.0.1
// either (a separate, unrelated OAuth feature). This plugin authenticates
// with a password only and never sets `oauthConfig`, so it's typed loosely
